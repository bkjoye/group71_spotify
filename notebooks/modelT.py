...
kT = int(dfTest.shape[0] / 6)
limit = 1000
n_cl = [2,10]
def func1() :
    print("starting 1")
    dfT = dfTest.iloc[0 : kT, :].sample(limit, random_state=0)
    dfP = pd.merge(df, dfT, left_on='playlist_id', right_on='playlist_id', how='left').dropna()
    print("running 1")
    generate_playlists(dfT, dfP, "test1", reg, try_clusters=n_cl, try_startNum=[10])
    print("done 1")
def func2() :
    print("starting 2")
    dfT = dfTest.iloc[kT : 2*kT, :].sample(limit, random_state=0)
    dfP = pd.merge(df, dfT, left_on='playlist_id', right_on='playlist_id', how='left').dropna()
    print("running 2")
    generate_playlists(dfT, dfP, "test2", reg, try_clusters=n_cl, try_startNum=[10])
    print("done 2")
def func3() :
    print("starting 3")
    dfT = dfTest.iloc[2*kT : 3*kT, :].sample(limit, random_state=0)
    dfP = pd.merge(df, dfT, left_on='playlist_id', right_on='playlist_id', how='left').dropna()
    print("running 3")
    generate_playlists(dfT, dfP, "test3", reg, try_clusters=n_cl, try_startNum=[10])
    print("done 3")
def func4() :
    print("starting 4")
    dfT = dfTest.iloc[3*kT : 4*kT, :].sample(limit, random_state=0)
    dfP = pd.merge(df, dfT, left_on='playlist_id', right_on='playlist_id', how='left').dropna()
    print("running 4")
    generate_playlists(dfT, dfP, "test4", reg, try_clusters=n_cl, try_startNum=[10])
    print("done 4")
def func5() :
    print("starting 5")
    dfT = dfTest.iloc[4*kT : 5*kT, :].sample(limit, random_state=0)
    dfP = pd.merge(df, dfT, left_on='playlist_id', right_on='playlist_id', how='left').dropna()
    print("running 5")
    generate_playlists(dfT, dfP, "test5", reg, try_clusters=n_cl, try_startNum=[10])
    print("done 5")
def func6() :
    print("starting 6")
    dfT = dfTest.iloc[5*kT :, :].sample(limit, random_state=0)
    dfP = pd.merge(df, dfT, left_on='playlist_id', right_on='playlist_id', how='left').dropna()
    print("running 6")
    generate_playlists(dfT, dfP, "test6", reg, try_clusters=n_cl, try_startNum=[10])
    print("done 6")
